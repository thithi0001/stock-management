...
const btnBase =
  "py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 transition duration-150 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed";
...
function ApprovalPage() {
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {receipts.map((receipt) => {
              const statusInfo = STATUS_CONFIG[receipt.receipt_status] || {
                text: receipt.receipt_status,
              ...
              return (
                <tr key={receipt.receipt_id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                    {receipt.created_by?.full_name || "N/A"}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
        ...
        <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl">
          <div className="flex justify-between items-center p-5 border-b border-gray-200">
            <h2 className="text-xl font-semibold">
              Chi tiết Phiếu xuất #{selectedReceipt.receipt_id}
            </h2>
            <button
              ...
              <>
                <div className="pb-4 mb-4 border-b border-gray-200">
                  <p className="text-sm text-gray-700 mb-1">
                    ...
                    {detailData.customer?.customer_name}
                  </p>
                  <p className="text-sm text-gray-700 mb-1">
                    <strong className="font-medium text-gray-900">
                      Địa chỉ:
                  ...
                  <p className="text-sm text-gray-700 mb-1">
                    <strong className="font-medium text-gray-900">SĐT:</strong>{" "}
                  ...
                  <p className="text-sm text-gray-700 mb-1">
                    ...
                    {detailData.created_by?.full_name}
                  </p>
                  <p className="text-sm text-gray-700 mb-1">
                    <strong className="font-medium text-gray-900">
                      Tổng tiền:
                  ...
                  <p className="text-sm text-gray-700 mb-1">
                    ...
                    {STATUS_CONFIG[detailData.receipt_status].text}
                  </p>
                  {detailData.approval_info && (
                    <>
                      <p className="text-sm text-gray-700 mb-1">
                        ...
                        {detailData.approval_info.approved_by_name}
                      </p>
                      <p className="text-sm text-gray-700 mb-1">
                        <strong className="font-medium text-gray-900">
                          Ngày duyệt:
                        </strong>{" "}
                        {formatDate(detailData.approval_info.approved_at)}
                      </p>
                      <p className="text-sm text-gray-700 mb-1">
                        <strong className="font-medium text-gray-900">
                          Ghi chú:
                        </strong>{" "}
                        {detailData.approval_info.reason}
                      </p>
                    </>
                  )}
                </div>
        ...
        >
          ...
          <div className="p-6">
            <p className="text-base text-gray-700">
              Bạn có chắc muốn <strong>{isReject ? "TỪ CHỐI" : "DUYỆT"}</strong>{" "}
              phiếu xuất <strong>#{selectedReceipt.receipt_id}</strong> không?
            </p>
            {isReject && (
              <div className="mt-4">
                <label
  ...
  return (
    <div className="p-6 min-h-screen">
      ...
      {renderTabs()}
      <div className="bg-white p-6 rounded-lg shadow-sm">{renderTable()}</div>
      {renderDetailModal()}
      {renderActionModal()}
    </div>
  );
